package com.sharp.chat.DragList;
...
public class DragListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
         ...
         */
        convertView = LayoutInflater.from(mContext).inflate(R.layout.listview_grouping, null);

        initItemView(position, convertView);

        TextView titleTv = (TextView) convertView.findViewById(R.id.grouping_lv_name);
        ...
        if (isChanged) {// 判断是否发生了改变
            ...
            if (position == mInvisilePosition) {
                ...
                if (!mShowItem) {// 在拖拽过程不允许显示的状态下，设置Item内容隐藏
                    ...
                    convertView.findViewById(R.id.grouping_item_layout)
                            .setBackgroundColor(0x0000000000);

                    // 隐藏Item上面的内容
                    int vis = View.INVISIBLE;
                    convertView.findViewById(R.id.grouping_lv_adj)
                            .setVisibility(vis);
                    convertView.findViewById(R.id.grouping_lv_del)
                            .setVisibility(vis);
